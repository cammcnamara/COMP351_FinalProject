...
# Rebuild clean_text if missing (Can maybe remove, added this because cell below wouldnt run)
train_df["clean_text"] = train_df["comment_text"].str.lower()
train_df["clean_text"] = train_df["clean_text"].apply(lambda x: re.sub(r"http\S+|www\S+|https\S+", "", x))
train_df["clean_text"] = train_df["clean_text"].apply(lambda x: re.sub(r"<.*?>", "", x))
train_df["clean_text"] = train_df["clean_text"].apply(lambda x: re.sub(r"[^a-zA-Z\s]", " ", x))
...
def clean_comment_inference(text):
    text = text.lower()
    text = re.sub(r"http\S+|www\S+|https\S+", "", text)
    text = re.sub(r"<.*?>", "", text)
    text = re.sub(r"[^a-zA-Z\s]", " ", text)
    text = " ".join([w for w in text.split() if w.isalpha() and w not in stop_words and len(w) > 1])
    return text



def make_nn_input(clean_text):
...
def classify_text_for_widget(text, threshold=0.3):
    cleaned = clean_comment_inference(text)
    X_vec = tfidf.transform([cleaned])

    print("Original comment:\n", text, "\n")
    print("Cleaned:\n", cleaned, "\n")
    print("Predictions (threshold = {:.2f}):\n".format(threshold))

    # ---------------- SVM ----------------
    try:
        svm_proba = []
        for model_svm in calibrated_svms:
            svm_proba.append(model_svm.predict_proba(X_vec)[0, 1])

        svm_proba = np.array(svm_proba)
        svm_preds = (svm_proba >= threshold).astype(int)

        print("--- SVM (Calibrated Linear SVM) ---")
    ...
    # ----------- NN Input -----------
    x_pad = make_nn_input(cleaned)

    # ---------------- BiLSTM ----------------
    try:
        lstm_proba = model.predict(x_pad, verbose=0)[0]
...
print("lstm model (model):", 'model' in globals())
print("lstm model (lmodel):", 'lmodel' in globals())
print("tokenizer:", 'tokenizer' in globals())
print("max_len:", 'max_len' in globals())
